...
1. **S-040: Plain Text, Markdown, and Bounded Clean-HTML Ingestion**
   - State: `COMPLETED` as `passed`
   - Owner: none
   - Scope: [S-040](s040_document_ingestion.md); GitHub issue #15 is closed.

## Next

S-041 through S-043 remain `PROVISIONAL`, inactive, and unauthorized pending a separate activation stage.

## Document Reader Productization Program

Human decision D-010 authorizes program entry after the recorded S-036 `ready` disposition. S-037 through S-040 completed as `passed`; no scope is active:
