...
S-038A is the provisional immediate successor to S-038 and predecessor to S-039. S-038A and S-039 through S-043 remain inactive and unauthorized in the strict order defined by the [Document Reader Productization Program](document_reader_productization_program.md).
...
2. **S-038: Minimal Browser Regression Baseline** — `AWAITING_HUMAN_VALIDATION`, owned by Human; [scope](s038_minimal_browser_regression_baseline.md); GitHub issue #13.
3. **S-038A: Parser CI Evaluation-Surface Coverage** — `PROVISIONAL`, inactive, and unauthorized; [scope](s038_parser_ci_evaluation_surface.md); GitHub issue #25, with issue #23 as bounded implementation work.
4. **S-039: Application-Service Boundary and Source-Document Contract** — `PROVISIONAL`; [scope](s039_application_service_source_document_contract.md). Depends on S-038A and includes tracked work item S-039A, GitHub issue #2.
5. **S-040: Plain Text, Markdown, and Bounded Clean-HTML Ingestion** — `PROVISIONAL`; [scope](s040_document_ingestion.md).
