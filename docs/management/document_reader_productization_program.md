...
The program is active under human authorization D-010, committed in `4baf3e8`, after S-036 completed with prototype disposition `ready`. S-037 through S-039 completed as `passed`. Only the scope named by [STATUS](STATUS.md) is authorized; S-040 is active and S-041 through S-043 remain provisional until separately activated.
...
Within the ordered program, S-039 completed the application-service boundary and project-owned source-document contract required before ingestion, continuity, and long-document features advance. S-040 now owns bounded local ingestion through that contract. D-008 continues to govern the separate architectural requirement that linguistic evidence reaching the RSVP optimizer remain backend-neutral.
...
5. **[S-040: Plain Text, Markdown, and Bounded Clean-HTML Ingestion](s040_document_ingestion.md)** — `AWAITING_HUMAN_VALIDATION`, owned by Human, and active under open GitHub issue #15.
...
- Only one scope may be active through [STATUS](STATUS.md); S-041 through S-043 remain provisional until separately activated. GitHub issue #24 is an authorized non-blocking follow-up and is not active.
